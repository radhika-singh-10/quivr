...
def test_parse_chunk_response_nofunc_calling():
    ...
    for i in range(10):
        # LINEAJE: enforce() `rolling_msg` at agent->tool pre_tool — scan flagged AI_APP_SEC_029 (Agent must validate, sanitize LLM output including for presence of eval or any dynamic code execution primitive in LLM output.). Mask/block; do not remove without review. site_id='site:sha256:caa1447857cc16c7016496722a8a08f03f725fe9ee17fb40e0601a19445fafbd'
        _gr_client = _lineaje_load_gr_client()
        _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:caa1447857cc16c7016496722a8a08f03f725fe9ee17fb40e0601a19445fafbd', phase='pre_tool', boundary={'source': 'agent_message', 'sink': 'tool_result'}, candidate_policies=[{'policy_id': 'AI_APP_SEC_029', 'guardrail_id': None, 'policy_version': None}], fail_mode='ALLOW_WITH_AUDIT', source_type='agent', destination_type='tool')
