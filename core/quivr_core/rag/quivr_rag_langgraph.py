...
class QuivrQARAGLangGraph:
    ...
    async def retrieve_full_documents_context(self, state: AgentState) -> AgentState:
        ...
        _lineaje_payload_838 = f"Top knowledge IDs: {top_knowledge_ids}"
        # LINEAJE: enforce() `_lineaje_payload_838` at agent->log log_emit — scan flagged AI_DAT_SEC_030 (Enforce minimum six-month log retention for high-risk AI systems). Mask/block; do not remove without review. site_id='site:sha256:68da78994b82bd5ee5f3d86b3f93cbf138199c430182672f66293e2344af5b43'
        ...
        _lineaje_payload_838 = await __import__('asyncio').to_thread(lambda: _gr_client.enforce(_gr_site, _lineaje_payload_838, content_type='application/json'))
        logger.info(_lineaje_payload_838)
    ...
    def reduce_rag_context(
        ...
        while n > max_context_tokens * SECURITY_FACTOR:
            ...
            if iteration > MAX_ITERATIONS:
                _lineaje_payload_932 = f"Attained the maximum number of iterations ({MAX_ITERATIONS})"
                # LINEAJE: enforce() `_lineaje_payload_932` at agent->log log_emit — scan flagged AI_DAT_SEC_030 (Enforce minimum six-month log retention for high-risk AI systems). Mask/block; do not remove without review. site_id='site:sha256:7d61ffcea9ce191a08dca54a978266822907119a04e548fcf8300d8b34ef1c75'
                _gr_client = _lineaje_load_gr_client()
                ...
                _lineaje_payload_932 = _gr_client.enforce(_gr_site, _lineaje_payload_932, content_type='application/json')
                logging.warning(
                    _lineaje_payload_932
                )
                break
    ...
    def generate_zendesk_rag(self, state: AgentState) -> AgentState:
        ...
        msg = _gr_client.enforce(_gr_site, msg, content_type='application/json', variable_name='msg', source_file=__file__, before_line=976)
    ...
    def generate_rag(self, state: AgentState) -> AgentState:
        ...
        msg = _gr_client.enforce(_gr_site, msg, content_type='application/json', variable_name='msg', source_file=__file__, before_line=988)
        response = llm.invoke(msg)
