        ...
        *,
        ...
        try:
            ...
            _gr_client.persist_runtime_mask_to_source(
                _lineaje_payload, source_file=__file__, variable_name='_lineaje_payload', before_line=348
            )
            ...
            _lineaje_logging.getLogger("lineaje.gr_client").warning(
                "Lineaje guardrail unavailable at site_id='site:sha256:f2691307f34833bb0a19cc007a6ae256946125c32bbecde051672c42abd5483f' (%s) — blocking (fail_mode=BLOCK)", _gr_exc
            )
            raise PermissionError(
                f"Lineaje guardrail unavailable at site_id='site:sha256:f2691307f34833bb0a19cc007a6ae256946125c32bbecde051672c42abd5483f' and fail_mode=BLOCK: {_gr_exc}"
            ) from _gr_exc
        ...
        _lineaje_payload_406 = f"added {len(docs)} chunks to vectordb"
        try:
            _gr_client = _lineaje_load_gr_client()
            _gr_site = _gr_client.SiteDescriptor(site_id='site:sha256:7f8e2cf5670152287c98d242c896ce5daed77fda50f07ff7527531230d2687e8', phase='log_emit', boundary={'source': 'log', 'sink': 'log'}, candidate_policies=[{'policy_id': 'AI_DAT_SEC_010', 'guardrail_id': 'Mask PII in Logs', 'policy_version': '2026.08.1'}], fail_mode='BLOCK', source_type='agent', destination_type='log')
            import asyncio as _gr_asyncio
            _gr_decision = await _gr_asyncio.to_thread(lambda: _gr_client.check(_gr_site, _lineaje_payload_406, content_type='application/json'))
            if _gr_decision.blocked:
                raise _gr_decision.as_error()
            _lineaje_payload_406 = _gr_decision.payload
            _gr_client.persist_runtime_mask_to_source(
                _lineaje_payload_406, source_file=__file__, variable_name='_lineaje_payload_406', before_line=406
            )
        except PermissionError:
            raise
        except Exception as _gr_exc:
            import logging as _lineaje_logging
            _lineaje_logging.getLogger("lineaje.gr_client").warning(
                "Lineaje guardrail unavailable at site_id='site:sha256:7f8e2cf5670152287c98d242c896ce5daed77fda50f07ff7527531230d2687e8' (%s) — blocking (fail_mode=BLOCK)", _gr_exc
            )
            raise PermissionError(
                f"Lineaje guardrail unavailable at site_id='site:sha256:7f8e2cf5670152287c98d242c896ce5daed77fda50f07ff7527531230d2687e8' and fail_mode=BLOCK: {_gr_exc}"
            ) from _gr_exc
        logger.debug(_lineaje_payload_406)
