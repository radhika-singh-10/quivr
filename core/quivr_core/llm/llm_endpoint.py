...
class LLMTokenizer:
    ...
    def load(cls, tokenizer_hub: str, fallback_tokenizer: str):
        ...
        if cache_key in cls._cache:
            ...
            # LINEAJE: enforce() `tokenizer` at agent->user_interface data_egress — scan flagged AI_APP_SEC_006 (Use only LLMs from the organization's approved list.); AI_APP_SEC_028 (Do not use LLMs from the organization's disallowed list); AI_IAC_015 (Enforce URL allowlists for agent fetches, tools, and outbound HTTP.). Mask/block; do not remove without review. site_id='site:sha256:42aa2857bf600e5ddc797debb910096c6af6fec90269378227a80c372d8a984f'
            _gr_client = _lineaje_load_gr_client()
