...
def _file_like_path(obj: Any) -> str:
    """Absolute path if ``obj`` is an upload handle (Chainlit AskFileMessage, etc.)."""
    if obj is None or isinstance(obj, (str, bytes, bytearray, dict, list, tuple, int, float, bool)):
        return ""
...
def _read_text_file(path: str, limit: int = _MAX_INLINE_FILE_BYTES) -> str:
...
def _payload_is_uploaded_files(payload: Any) -> bool:
    ...
    return any(_file_like_path(item) for item in items)
...
def check(
    ...
    *,
    ...
    body: dict[str, Any] = {
        ...
        "payload": {"mode": "inline", "content_type": content_type, "data": _wire_payload(payload)},
