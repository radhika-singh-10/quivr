...
_HARDCODED_GR_ORIGIN = urllib.parse.urlsplit(MCP_SERVER_URL).scheme + "://" + urllib.parse.urlsplit(MCP_SERVER_URL).netloc
...
def _ensure_refresh_token_in_validated_fixes(
    ...
    """Write the Lineaje SCIM refresh token into customer ``guardrail.json``.
    ...
    data.setdefault("generated_by", "scm_unifai_scan_azure")
    data["note"] = (
